...
export default function DriverDashboard() {
  ...
  const getStatusButtonLabel = (status: GuestStatus): string => {
    switch (status) {
      case "assigned":
        return t("driver.markAsArrived")
      case "picked-up":
        return t("driver.markAsCompleted")
      default:
        return ""
    }
  }
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {t("driver.activeTransfersAssigned").replace("{count}", tasks.length.toString())}
        </p>
      </div>
      ...
      <div className="space-y-4">
        {tasks.length > 0 ? (
          tasks.map((task) => {
            const event: { name: string } | null = null // getEventById(task.eventId)
                ...
                <CardContent className="space-y-4 pt-4">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
                    ...
                    <Button asChild size="sm" variant="outline">
                      <a href={`tel:${task.phone}`}>{t("driver.call")}</a>
                    </Button>
                  </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {completedTasks.map((task) => {
              const event: any = null // getEventById(task.eventId)
              return (
                <div
                  key={task.id}
