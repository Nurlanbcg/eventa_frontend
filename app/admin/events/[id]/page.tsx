  ...
  const handleAssignDriver = async (driver: Driver) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Format SMS
        const sms = `Your VIP transfer is arranged. Pickup: ${selectedGuest.pickupAddress}. Time: ${event.time}. Driver: ${driver.name} (${driver.phone}). Vehicle: ${driver.vehicleModel}.`
        setSmsPreview(sms)
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mb-2">
                        <span className="font-medium text-foreground">Pickup:</span> {guest.pickupAddress}
                      </p>
                      {driver ? (
                        <button
                          className="text-xs text-muted-foreground flex items-center gap-1 hover:text-accent transition-colors text-left w-full"
                          onClick={() => {
                            setSelectedGuest(guest)
                            setIsAssignDriverOpen(true)
                          }}
      ...
      <Dialog open={!!smsPreview} onOpenChange={() => setSmsPreview(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This message will be sent to the guest
            </DialogDescription>
          </DialogHeader>
